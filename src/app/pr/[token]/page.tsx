    ...
    <div style={{ minHeight: "100dvh", background: "#0b0f0e", color: "#e8ece9", fontFamily: "system-ui, -apple-system, 'Noto Sans JP', 'Noto Sans KR', sans-serif" }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "28px 20px 60px" }}>
        ...
        {state === "ok" && deal && (<>
          ...
          {/* 의뢰 내용 — AI 요약 + 원문 + 붙여넣기 (상단 배치, 펼치기/접기) */}
          <Section title={t.brief} id="sec-brief" collapsible defaultOpen labels={{ open: t.expand, close: t.collapse }}>
...
function Section({ title, children, id, collapsible, defaultOpen = true, labels }: { title: string; children: React.ReactNode; id?: string; collapsible?: boolean; defaultOpen?: boolean; labels?: { open: string; close: string } }) {
  ...
  return (
    <div id={id} style={{ background: "#121715", border: "1px solid #212824", borderRadius: 16, padding: 20, marginBottom: 16, scrollMarginTop: 70 }}>
      <div
        ...
        style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: open ? 12 : 0, cursor: collapsible ? "pointer" : "default", userSelect: "none" }}
      >
        <span style={{ fontSize: 12.5, fontWeight: 800, color: "#8a938d", letterSpacing: ".02em" }}>{title}</span>
        {collapsible && (
          <span style={{ marginLeft: "auto", fontSize: 11.5, fontWeight: 700, color: "#3fb984", display: "flex", alignItems: "center", gap: 4 }}>
            {open ? labels?.close : labels?.open}<span style={{ transform: open ? "rotate(180deg)" : "none", transition: "transform .15s" }}>▾</span>
          </span>
        )}
      </div>
